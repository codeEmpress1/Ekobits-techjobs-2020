...
// 2
const printHobbies = () => users.forEach(user => console.log(user.hobbies))
...
function allLanguages() {
  let ans = users.reduce((acc, user) => acc.concat(user.favoriteLanguages), []);
...
function removeVowel(str) {
  ...
  let stripedArr = arr.map(char => /[aeiouAEIOU]/.test(char) ? char = '' : char)
  let ans = stripedArr.join('');
  return ans;
}
...
function getDurationsInSeconds(songArr) {
  return songArr.map(function(song) {
      ...
      let minutes = +durationNums[0];
      let seconds = +durationNums[1];
